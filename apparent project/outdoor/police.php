...
if(isset($_POST["addbtn"])){
    ...
    if(empty($fname) or empty($username) or (empty($password) and empty($repassword)) ){
        if(empty($fname)){
  ...
  elseif(count($err)==0){
    ...
    $query="insert into enter (fullname, username, password) value ('$fname','$username','$password')";
...
if(isset($_POST['deletebtn'])){
    ...
    $sql = "select * from enter where username='" . $search . "'";
    $query = mysqli_query($conn, $sql);
    if(mysqli_num_rows($query) == false){
      $errr = "Dhifama! Maqaa Fayyadamaa kun hin galmoofnee";
    }
    else{
        $sql = "delete from enter where username='" . $search . "'";
    $query = mysqli_query($conn, $sql);
    $congra="Deleted!";
    }
}
if(isset($_POST["updatebtn"])){
  ...
  elseif(count($err)==0){
    ...
    $query="update enter set fullname='" . $_POST["adminname"] . "',username='" . $_POST["adminusername"] . "',password='" . $_POST["adminpass"] . "' where username='" . $search . "'  ";
    mysqli_query($conn,$query);
    ...
    <div class="box">
      ...
      <div class="form">
        <form action="" method="post">
           <div style="width: 100%;"><a href="index.php">Logout</a><a href="addadmin.php">Add Director</a></div>
            <div class="textform">
                <ul>
                    <li><span>Full Name:</span><input type="text" name="adminname" value="<?php echo $funame; ?>" placeholder="Enter Police Name">
                </li>
                <li><span>Username:</span><input type="text" name="adminusername" value="<?php echo $uname; ?>" placeholder="Enter Police username">
                 </li>
